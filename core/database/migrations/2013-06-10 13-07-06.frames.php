...
class build_frames extends base_build
{
    private $_builder;
    ...
    public function __Construct ( $db_name, $tablename )
    {
        ...
        $this->_build = new query_builder ( $db_name, "frames" );
    }

    public function put ()
    {
        $this->_build->create_table ( "frames" );

        $this->_build->varchar ( "title", "255" );
        $this->_build->varchar ( "price", "255" );
        $this->_build->varchar ( "doors_id", "255" );
...
$build = new build_frames ( $this->_db_name, "frames" );
$build->desc ();
