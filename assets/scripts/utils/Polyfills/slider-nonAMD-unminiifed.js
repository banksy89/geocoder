var fdSlider = (function() {
    ...
    var init = function() {
            ...
            for(var i = 0, inp; inp = inputs[i]; i++) {
                    // Checks which elements to replace (but we've extended it so it doesn't convert elements that have a class of 'js-ignore-input')
                    if (inp.className.indexOf('js-ignore-input') !== -1) {
                        continue;
                    }
                    ...
                    console.group();
                        ...
                        console.log(inp);
                        console.log('is it an input: ', inp.tagName.toLowerCase() == "input");
                        console.log('is it a txt input: ', inp.type.toLowerCase() == "text");
                        console.log('is it a range input: ', inp.type.toLowerCase() == "range");
                        console.log('does it have one of the required attributes: ', !!(
                        ...
                        console.log('if so then which one (min): ', !!(getAttribute(inp, "min") && getAttribute(inp, "min").search(fpRegExp) != -1));
                        console.log('if so then which one (max): ', !!(getAttribute(inp, "max") && getAttribute(inp, "max").search(fpRegExp) != -1));
                        console.log('if so then which one (step): ', !!(getAttribute(inp, "step") && getAttribute(inp, "step").search(/^(any|([0-9]+(\.[0-9]+){0,1}))$/i) != -1));
                        console.log('overall result: ', !!(inp.tagName.toLowerCase() == "input" && inp.type.toLowerCase() == "text" && 
                            (
                                getAttribute(inp, "min") && getAttribute(inp, "min").search(fpRegExp) != -1 ||
                                getAttribute(inp, "max") && getAttribute(inp, "max").search(fpRegExp) != -1 ||
                                getAttribute(inp, "step") && getAttribute(inp, "step").search(/^(any|([0-9]+(\.[0-9]+){0,1}))$/i) != -1
                    ...
                    */
                    ...
                    /*
                        if statement matches true if...

                        The tab is an input,
                        and is a text input
                        and has either a min, or max, or step value

                        BUT IVE UPDATED IT TO IGNORE THE NATIVE CONTROL SO EVEN HTML5 COMPATIBLE BROWSERS GET THE CUSTOM BUILT CONTROLS
                     */
                    if(inp.tagName.toLowerCase() == "input" && (inp.type.toLowerCase() == "text" || inp.type.toLowerCase() == "range") && 
                            (
                                getAttribute(inp, "min") && getAttribute(inp, "min").search(fpRegExp) != -1 ||
                                getAttribute(inp, "max") && getAttribute(inp, "max").search(fpRegExp) != -1 ||
                                getAttribute(inp, "step") && getAttribute(inp, "step").search(/^(any|([0-9]+(\.[0-9]+){0,1}))$/i) != -1
